const url =
    "https://pixabay.com/api/?key=23641870-0eadc2c11d04a3e326b69b23d&page=1&per_page=12";
...
function paginationControl(type) {
    ...
    paginationWrapper.innerHTML = `
        ${
            pageCounter <= 1
                ? `<span class="page_number" id="page_left"></span>`
                : `<span onclick="paginationControl('back')" class="page_number" id="page_left">&larr;</span>`
        }
        ...
        ${
            pageCounter == totalPages
                ? `<span class="page_number" id="page_right"></span>`
                : `<span onclick="paginationControl('fwd')" class="page_number" id="page_right">&rarr;</span>`
        }
    `;
...
function sendRequest(pageNumber = 1, initialLoad) {
    ...
    fetch(
        url +
            `&q=${localStorage.getItem("lastRequest")}&order=${
                extQuery1 || "popular"
        ...
        .then((response2) => {
            ...
            contentObject.forEach((elementObject) => {
                ...
                let src = elementObject.webformatURL;
                let image = new Image();
